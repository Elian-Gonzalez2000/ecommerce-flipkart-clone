...
import { getProductBySearchQuery, getProductsBySlug } from "../../actions";
...
function SearchProducts() {
  ...
  useEffect(() => {
    dispatch(getProductBySearchQuery(params.q));
    //console.log(params);
  }, [params.q]);
  ...
  if (productData.products.length < 1 && productData.loading == false)
    return NoResultsFounded();
  return (
    <Layout>
      <div className="products-list-show">
        <aside>
          <div>
          ...
          <div style={{ marginBottom: "0.5rem" }} className="pagination-info">
            {productData.products.length > 0 ? (
              <>
                ...
                <span>
                  (Showing {start + 1} –{" "}
                  {end < productData.products.length
                    ? end
                    : productData.products.length}{" "}
                  products of {productData.products.length} products)
                </span>
              </>
            ) : (
              <Skeleton
          ...
          <div className="sort-by">
            {productData.products.length > 0 ? (
              <>
                ...
                <Link
                  to={`${pathname}?q=${params.cid}&sort=popularity`}
                  className={getActiveSortClass("popularity")}
                >
                  Popularity
                </Link>{" "}
                <Link
                  to={`${pathname}?q=${params.cid}&sort=price_asc`}
                  className={getActiveSortClass("price_asc")}
                >
                  Price -- Low to High
                </Link>{" "}
                <Link
                  to={`${pathname}?q=${params.cid}&sort=price_desc`}
                  className={getActiveSortClass("price_desc")}
                >
                  Price -- High to Low
                </Link>{" "}
                <Link
                  to={`${pathname}?q=${params.cid}&sort=recency_desc`}
                  ...
                  <Card header={false} key={randomUI()}>
                    ...
                    >
                      ...
                      <div className="card-content">
                        <div>
                          <b style={{ fontWeight: "normal" }}>{prod.name}</b>
                          <picture>
